...
- [x] **EditTab (수정) & EventEditPage**:
    - [x] 기준일 제거 (항상 오늘).
    - [x] **스타일 개선 2차**: 배경색 통일, 테두리 제거, 간격 확보, 날짜 볼드 제거, 삭제 버튼 강조(회색).
- [x] **DiaryTab (다이어리)**:
    - [x] **데이터 동기화 버그 수정**: `eventId` 기반 구독으로 변경하여 작성 시 즉시 반영 보장.
    - [x] **스타일 개선**:
        - [x] 카드 테두리 및 달력 아이콘 제거.
        - [x] 작성 다이얼로그 UX 개선 (디자인 폴리싱, M3 스타일).
        - [x] FAB 스타일 통일.
    - [x] 카드 삭제: 스와이프(Dismissible) 적용.
    - [x] 카드 수정: 디자인 개선된 카드 클릭 수정.
- [x] **TodoTab (할일)**:
    - [x] 입력 필드 및 Empty State 스타일 개선.
