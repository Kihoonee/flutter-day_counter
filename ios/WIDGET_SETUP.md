...
3. Product Name: `DaysPlusWidget` 입력
4. "Include Live Activity" 및 "Include Configuration Intent"가 선택되어 있다면 체크를 해제합니다.
5. **Finish**를 클릭합니다.
6. 스킴(Scheme) 활성화 여부를 묻는 창이 뜨면 **Activate**를 클릭합니다.

### 3. Swift 코드 적용
1. Xcode에서 생성된 `DaysPlusWidget/DaysPlusWidget.swift` 파일을 찾습니다.
2. 해당 파일의 전체 내용을 프로젝트의 `ios/DaysPlusWidget/DaysPlusWidget.swift` 파일 내용으로 교체하거나 확인합니다.
...
1. **Runner 타겟**:
    ...
    *   App Groups 섹션에서 `+`를 클릭 -> `group.com.handroom.daysplus`를 추가합니다. (체크박스가 활성화되어 있는지 확인하세요).
2. **DaysPlusWidget 타겟**:
    *   `DaysPlusWidget` 타겟 선택 -> **Signing & Capabilities** 탭으로 이동합니다.
    *   `+ Capability` 클릭 -> **App Groups**를 추가합니다.
    *   `+`를 클릭하거나 기존의 `group.com.handroom.daysplus`를 선택합니다. (체크박스가 활성화되어 있는지 확인하세요).
